...
@implementation MSGraphListRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphList *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphList alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphList *)list
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[list dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:list]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphList alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
