...
@implementation MSGraphInvitationRequest
...
- (NSMutableURLRequest *)update:(MSGraphInvitation *)invitation
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[invitation dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:invitation]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphInvitation alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
