...
@implementation MSGraphPlannerTaskDetailsRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphPlannerTaskDetails *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlannerTaskDetails alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphPlannerTaskDetails *)plannerTaskDetails
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[plannerTaskDetails dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:plannerTaskDetails]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlannerTaskDetails alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
