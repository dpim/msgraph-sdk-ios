...
@implementation MSGraphWorkbookRangeFormatRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookRangeFormat *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookRangeFormat alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookRangeFormat *)workbookRangeFormat
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookRangeFormat dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookRangeFormat]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookRangeFormat alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
