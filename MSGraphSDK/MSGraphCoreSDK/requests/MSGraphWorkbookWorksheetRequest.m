...
@implementation MSGraphWorkbookWorksheetRequest
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookWorksheet *)workbookWorksheet
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookWorksheet dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookWorksheet]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookWorksheet alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
