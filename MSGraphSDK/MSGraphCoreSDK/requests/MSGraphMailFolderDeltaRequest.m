...
@implementation MSGraphMailFolderDeltaRequest
...
- (MSURLSessionDataTask *)executeWithCompletion:(void (^)(MSCollection *response, MSGraphMailFolderDeltaRequest *nextRequest, NSError *error))completionHandler
{
    ...
    MSURLSessionDataTask *task = [self collectionTaskWithRequest:self.mutableRequest
                                          odObjectWithDictionary:^(id responseObject){
                                                                     return [[MSGraphMailFolder alloc] initWithDictionary:responseObject];
                                                                 }
                                                      completion:^(MSCollection *collectionResponse, NSError *error){
                                      if(!error && collectionResponse.nextLink && completionHandler){
                                              MSGraphMailFolderDeltaRequest *nextRequest = [[MSGraphMailFolderDeltaRequest alloc] initWithURL:collectionResponse.nextLink
                                                                                                                  options:nil
                                                                                                                  client:self.client];
