...
@implementation MSGraphWorkbookChartTitleFormatRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookChartTitleFormat *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartTitleFormat alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookChartTitleFormat *)workbookChartTitleFormat
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookChartTitleFormat dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookChartTitleFormat]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartTitleFormat alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
