...
@implementation MSGraphWorkbookFunctionResultRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookFunctionResult *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookFunctionResult alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookFunctionResult *)workbookFunctionResult
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookFunctionResult dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookFunctionResult]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookFunctionResult alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
