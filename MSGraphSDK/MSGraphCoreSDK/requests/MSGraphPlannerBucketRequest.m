...
@implementation MSGraphPlannerBucketRequest
...
- (NSMutableURLRequest *)update:(MSGraphPlannerBucket *)plannerBucket
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[plannerBucket dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:plannerBucket]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlannerBucket alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
