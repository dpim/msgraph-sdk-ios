...
@implementation MSGraphPlannerPlanRequest
...
- (NSMutableURLRequest *)update:(MSGraphPlannerPlan *)plannerPlan
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[plannerPlan dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:plannerPlan]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlannerPlan alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
