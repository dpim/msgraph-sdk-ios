...
@implementation MSGraphPlannerRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphPlanner *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlanner alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphPlanner *)planner
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[planner dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:planner]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlanner alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
