...
@implementation MSGraphOnenoteEntityHierarchyModelRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphOnenoteEntityHierarchyModel *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOnenoteEntityHierarchyModel alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphOnenoteEntityHierarchyModel *)onenoteEntityHierarchyModel
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[onenoteEntityHierarchyModel dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:onenoteEntityHierarchyModel]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOnenoteEntityHierarchyModel alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
