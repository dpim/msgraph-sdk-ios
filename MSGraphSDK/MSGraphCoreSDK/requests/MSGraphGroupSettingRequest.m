...
@implementation MSGraphGroupSettingRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphGroupSetting *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphGroupSetting alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphGroupSetting *)groupSetting
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[groupSetting dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:groupSetting]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphGroupSetting alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
