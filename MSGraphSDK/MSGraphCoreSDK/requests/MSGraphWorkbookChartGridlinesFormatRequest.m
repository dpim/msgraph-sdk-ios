...
@implementation MSGraphWorkbookChartGridlinesFormatRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookChartGridlinesFormat *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartGridlinesFormat alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookChartGridlinesFormat *)workbookChartGridlinesFormat
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookChartGridlinesFormat dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookChartGridlinesFormat]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartGridlinesFormat alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
