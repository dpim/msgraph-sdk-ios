...
@implementation MSGraphDomainDnsSrvRecordRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphDomainDnsSrvRecord *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDomainDnsSrvRecord alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphDomainDnsSrvRecord *)domainDnsSrvRecord
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[domainDnsSrvRecord dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:domainDnsSrvRecord]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDomainDnsSrvRecord alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
