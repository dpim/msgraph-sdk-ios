...
@implementation MSGraphWorkbookTableRequest
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookTable *)workbookTable
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookTable dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookTable]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookTable alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
