...
@implementation MSGraphSharedDriveItemRequest
...
- (NSMutableURLRequest *)update:(MSGraphSharedDriveItem *)sharedDriveItem
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[sharedDriveItem dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:sharedDriveItem]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSharedDriveItem alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
