...
@implementation MSGraphWorkbookChartAxesRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookChartAxes *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartAxes alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookChartAxes *)workbookChartAxes
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookChartAxes dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookChartAxes]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartAxes alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
