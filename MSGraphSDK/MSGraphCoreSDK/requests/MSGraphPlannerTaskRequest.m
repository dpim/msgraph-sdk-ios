...
@implementation MSGraphPlannerTaskRequest
...
- (NSMutableURLRequest *)update:(MSGraphPlannerTask *)plannerTask
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[plannerTask dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:plannerTask]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlannerTask alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
