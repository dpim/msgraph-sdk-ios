...
@implementation MSGraphWorkbookChartDataLabelFormatRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookChartDataLabelFormat *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartDataLabelFormat alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookChartDataLabelFormat *)workbookChartDataLabelFormat
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookChartDataLabelFormat dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookChartDataLabelFormat]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartDataLabelFormat alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
