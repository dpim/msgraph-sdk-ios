...
@implementation MSGraphWorkbookChartAxisTitleRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookChartAxisTitle *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartAxisTitle alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookChartAxisTitle *)workbookChartAxisTitle
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookChartAxisTitle dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookChartAxisTitle]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartAxisTitle alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
