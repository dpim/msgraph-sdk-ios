...
@implementation MSGraphPlannerBucketTaskBoardTaskFormatRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphPlannerBucketTaskBoardTaskFormat *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlannerBucketTaskBoardTaskFormat alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphPlannerBucketTaskBoardTaskFormat *)plannerBucketTaskBoardTaskFormat
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[plannerBucketTaskBoardTaskFormat dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:plannerBucketTaskBoardTaskFormat]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlannerBucketTaskBoardTaskFormat alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
