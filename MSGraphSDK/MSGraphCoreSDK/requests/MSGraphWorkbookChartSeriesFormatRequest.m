...
@implementation MSGraphWorkbookChartSeriesFormatRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookChartSeriesFormat *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartSeriesFormat alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookChartSeriesFormat *)workbookChartSeriesFormat
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookChartSeriesFormat dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookChartSeriesFormat]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartSeriesFormat alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
