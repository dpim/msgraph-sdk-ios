...
@implementation MSGraphOnenoteEntityBaseModelRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphOnenoteEntityBaseModel *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOnenoteEntityBaseModel alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphOnenoteEntityBaseModel *)onenoteEntityBaseModel
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[onenoteEntityBaseModel dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:onenoteEntityBaseModel]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOnenoteEntityBaseModel alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
