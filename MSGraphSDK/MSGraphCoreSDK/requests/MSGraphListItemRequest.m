...
@implementation MSGraphListItemRequest
...
- (NSMutableURLRequest *)update:(MSGraphListItem *)listItem
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[listItem dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:listItem]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphListItem alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
