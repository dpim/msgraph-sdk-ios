...
@implementation MSGraphDomainRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphDomain *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDomain alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphDomain *)domain
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[domain dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:domain]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDomain alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
