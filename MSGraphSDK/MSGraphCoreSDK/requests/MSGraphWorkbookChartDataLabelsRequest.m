...
@implementation MSGraphWorkbookChartDataLabelsRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbookChartDataLabels *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartDataLabels alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbookChartDataLabels *)workbookChartDataLabels
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookChartDataLabels dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookChartDataLabels]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbookChartDataLabels alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
