...
@implementation MSGraphOnenoteRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphOnenote *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOnenote alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphOnenote *)onenote
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[onenote dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:onenote]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOnenote alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
