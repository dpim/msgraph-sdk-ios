...
- (comDomainState*) state
{
    if(!_state){
        _state = [[comDomainState alloc] initWithDictionary: self.dictionary[@"state"]];
    }
    return _state;
}

- (void) setState: (comDomainState*) val
{
    _state = val;
...
- (NSArray*) serviceConfigurationRecords
{
    ...
    if ([serviceConfigurationRecords isKindOfClass:[NSArray class]]){
        for (id domainDnsRecord in serviceConfigurationRecords){
            [serviceConfigurationRecordsResult addObject:[[comDomainDnsRecord alloc] initWithDictionary: domainDnsRecord]];
        }
    }
...
- (NSArray*) verificationDnsRecords
{
    ...
    if ([verificationDnsRecords isKindOfClass:[NSArray class]]){
        for (id domainDnsRecord in verificationDnsRecords){
            [verificationDnsRecordsResult addObject:[[comDomainDnsRecord alloc] initWithDictionary: domainDnsRecord]];
        }
    }
...
- (NSArray*) domainNameReferences
{
    ...
    if ([domainNameReferences isKindOfClass:[NSArray class]]){
        for (id directoryObject in domainNameReferences){
            [domainNameReferencesResult addObject:[[comDirectoryObject alloc] initWithDictionary: directoryObject]];
        }
    }
