...
@interface comSharingInvitation()
{
    NSString* _email;
    comIdentitySet* _invitedBy;
    NSString* _redeemedBy;
    BOOL _signInRequired;
}
@end

@implementation comSharingInvitation
...
- (comIdentitySet*) invitedBy
{
    if(!_invitedBy){
        _invitedBy = [[comIdentitySet alloc] initWithDictionary: self.dictionary[@"invitedBy"]];
    }
    return _invitedBy;
}

- (void) setInvitedBy: (comIdentitySet*) val
{
    _invitedBy = val;
