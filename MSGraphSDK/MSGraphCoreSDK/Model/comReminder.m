// Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.



#import "NSDate+MSSerialization.h"

#import "comModels.h"

@interface MSObject()

@property (strong, nonatomic) NSMutableDictionary *dictionary;

@end

@interface comReminder()
{
    NSString* _eventId;
    comDateTimeTimeZone* _eventStartTime;
    comDateTimeTimeZone* _eventEndTime;
    NSString* _changeKey;
    NSString* _eventSubject;
    comLocation* _eventLocation;
    NSString* _eventWebLink;
    comDateTimeTimeZone* _reminderFireTime;
}
@end

@implementation comReminder

- (NSString*) eventId
{
    if([[NSNull null] isEqual:self.dictionary[@"eventId"]])
    {
        return nil;
    }   
    return self.dictionary[@"eventId"];
}

- (void) setEventId: (NSString*) val
{
    self.dictionary[@"eventId"] = val;
}

- (comDateTimeTimeZone*) eventStartTime
{
    if(!_eventStartTime){
        _eventStartTime = [[comDateTimeTimeZone alloc] initWithDictionary: self.dictionary[@"eventStartTime"]];
    }
    return _eventStartTime;
}

- (void) setEventStartTime: (comDateTimeTimeZone*) val
{
    _eventStartTime = val;
    self.dictionary[@"eventStartTime"] = val;
}

- (comDateTimeTimeZone*) eventEndTime
{
    if(!_eventEndTime){
        _eventEndTime = [[comDateTimeTimeZone alloc] initWithDictionary: self.dictionary[@"eventEndTime"]];
    }
    return _eventEndTime;
}

- (void) setEventEndTime: (comDateTimeTimeZone*) val
{
    _eventEndTime = val;
    self.dictionary[@"eventEndTime"] = val;
}

- (NSString*) changeKey
{
    if([[NSNull null] isEqual:self.dictionary[@"changeKey"]])
    {
        return nil;
    }   
    return self.dictionary[@"changeKey"];
}

- (void) setChangeKey: (NSString*) val
{
    self.dictionary[@"changeKey"] = val;
}

- (NSString*) eventSubject
{
    if([[NSNull null] isEqual:self.dictionary[@"eventSubject"]])
    {
        return nil;
    }   
    return self.dictionary[@"eventSubject"];
}

- (void) setEventSubject: (NSString*) val
{
    self.dictionary[@"eventSubject"] = val;
}

- (comLocation*) eventLocation
{
    if(!_eventLocation){
        _eventLocation = [[comLocation alloc] initWithDictionary: self.dictionary[@"eventLocation"]];
    }
    return _eventLocation;
}

- (void) setEventLocation: (comLocation*) val
{
    _eventLocation = val;
    self.dictionary[@"eventLocation"] = val;
}

- (NSString*) eventWebLink
{
    if([[NSNull null] isEqual:self.dictionary[@"eventWebLink"]])
    {
        return nil;
    }   
    return self.dictionary[@"eventWebLink"];
}

- (void) setEventWebLink: (NSString*) val
{
    self.dictionary[@"eventWebLink"] = val;
}

- (comDateTimeTimeZone*) reminderFireTime
{
    if(!_reminderFireTime){
        _reminderFireTime = [[comDateTimeTimeZone alloc] initWithDictionary: self.dictionary[@"reminderFireTime"]];
    }
    return _reminderFireTime;
}

- (void) setReminderFireTime: (comDateTimeTimeZone*) val
{
    _reminderFireTime = val;
    self.dictionary[@"reminderFireTime"] = val;
}

@end
