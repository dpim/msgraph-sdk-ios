...
@interface comContact()
{
    ...
    comPhysicalAddress* _homeAddress;
    comPhysicalAddress* _businessAddress;
    comPhysicalAddress* _otherAddress;
...
- (NSArray*) emailAddresses
{
    ...
    if ([emailAddresses isKindOfClass:[NSArray class]]){
        for (id emailAddress in emailAddresses){
            [emailAddressesResult addObject:[[comEmailAddress alloc] initWithDictionary: emailAddress]];
        }
    }
...
- (comPhysicalAddress*) homeAddress
{
    if(!_homeAddress){
        _homeAddress = [[comPhysicalAddress alloc] initWithDictionary: self.dictionary[@"homeAddress"]];
    }
    return _homeAddress;
}

- (void) setHomeAddress: (comPhysicalAddress*) val
{
    _homeAddress = val;
    self.dictionary[@"homeAddress"] = val;
}

- (comPhysicalAddress*) businessAddress
{
    if(!_businessAddress){
        _businessAddress = [[comPhysicalAddress alloc] initWithDictionary: self.dictionary[@"businessAddress"]];
    }
    return _businessAddress;
}

- (void) setBusinessAddress: (comPhysicalAddress*) val
{
    _businessAddress = val;
    self.dictionary[@"businessAddress"] = val;
}

- (comPhysicalAddress*) otherAddress
{
    if(!_otherAddress){
        _otherAddress = [[comPhysicalAddress alloc] initWithDictionary: self.dictionary[@"otherAddress"]];
    }
    return _otherAddress;
}

- (void) setOtherAddress: (comPhysicalAddress*) val
{
    _otherAddress = val;
