// Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.



@class comTimeSlot, comAttendeeAvailability, comLocation; 
#import <UIKit/UiKit.h>
#import "comFreeBusyStatus.h"


#import "MSObject.h"

@interface comMeetingTimeSuggestion : MSObject

@property (nullable, nonatomic, setter=setMeetingTimeSlot:, getter=meetingTimeSlot) comTimeSlot* meetingTimeSlot;
@property (nonatomic, setter=setConfidence:, getter=confidence) CGFloat confidence;
@property (nullable, nonatomic, setter=setOrganizerAvailability:, getter=organizerAvailability) comFreeBusyStatus* organizerAvailability;
@property (nullable, nonatomic, setter=setAttendeeAvailability:, getter=attendeeAvailability) NSArray* attendeeAvailability;
@property (nullable, nonatomic, setter=setLocations:, getter=locations) NSArray* locations;
@property (nullable, nonatomic, setter=setSuggestionReason:, getter=suggestionReason) NSString* suggestionReason;

@end
