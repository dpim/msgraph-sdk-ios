...
@interface comMessage()
{
    ...
    comItemBody* _uniqueBody;
    BOOL _isDeliveryReceiptRequested;
    BOOL _isReadReceiptRequested;
    BOOL _isRead;
    BOOL _isDraft;
    NSString* _webLink;
    comInferenceClassificationType* _inferenceClassification;
...
- (NSArray*) toRecipients
{
    ...
    if ([toRecipients isKindOfClass:[NSArray class]]){
        for (id recipient in toRecipients){
            [toRecipientsResult addObject:[[comRecipient alloc] initWithDictionary: recipient]];
        }
    }
...
- (NSArray*) bccRecipients
{
    ...
    if ([bccRecipients isKindOfClass:[NSArray class]]){
        for (id recipient in bccRecipients){
            [bccRecipientsResult addObject:[[comRecipient alloc] initWithDictionary: recipient]];
        }
    }
...
- (NSArray*) replyTo
{
    ...
    if ([replyTo isKindOfClass:[NSArray class]]){
        for (id recipient in replyTo){
            [replyToResult addObject:[[comRecipient alloc] initWithDictionary: recipient]];
        }
    }
...
- (comItemBody*) uniqueBody
{
    if(!_uniqueBody){
        _uniqueBody = [[comItemBody alloc] initWithDictionary: self.dictionary[@"uniqueBody"]];
    }
    return _uniqueBody;
}

- (void) setUniqueBody: (comItemBody*) val
{
    _uniqueBody = val;
...
- (comInferenceClassificationType*) inferenceClassification
{
    if(!_inferenceClassification){
        _inferenceClassification = [self.dictionary[@"inferenceClassification"] tocomInferenceClassificationType];
    }
    return _inferenceClassification;
}

- (void) setInferenceClassification: (comInferenceClassificationType*) val
{
