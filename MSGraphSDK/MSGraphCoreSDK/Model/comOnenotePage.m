...
@interface comOnenotePage()
{
    ...
    int32_t _order;
    NSArray* _userTags;
    comOnenoteSection* _parentSection;
    comNotebook* _parentNotebook;
}
@end

@implementation comOnenotePage
...
{
    if(!_links){
        _links = [[comPageLinks alloc] initWithDictionary: self.dictionary[@"links"]];
    }
    return _links;
}

- (void) setLinks: (comPageLinks*) val
{
    _links = val;
...
- (comOnenoteSection*) parentSection
{
    if(!_parentSection){
        _parentSection = [[comOnenoteSection alloc] initWithDictionary: self.dictionary[@"parentSection"]];
    }
    return _parentSection;
}

- (void) setParentSection: (comOnenoteSection*) val
{
    _parentSection = val;
