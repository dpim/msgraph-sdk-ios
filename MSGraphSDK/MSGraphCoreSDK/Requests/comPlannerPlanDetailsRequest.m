...
@implementation comPlannerPlanDetailsRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(comPlannerPlanDetails *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comPlannerPlanDetails alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(comPlannerPlanDetails *)plannerPlanDetails
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[plannerPlanDetails dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:plannerPlanDetails]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comPlannerPlanDetails alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
