...
@implementation comWorkbookTableSortRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(comWorkbookTableSort *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comWorkbookTableSort alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(comWorkbookTableSort *)workbookTableSort
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookTableSort dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookTableSort]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comWorkbookTableSort alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
