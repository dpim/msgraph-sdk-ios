...
@implementation comWorkbookFunctionsRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(comWorkbookFunctions *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comWorkbookFunctions alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(comWorkbookFunctions *)workbookFunctions
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookFunctions dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookFunctions]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comWorkbookFunctions alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
