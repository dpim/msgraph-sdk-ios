...
@implementation comGroupDeltaRequest
...
- (MSURLSessionDataTask *)executeWithCompletion:(void (^)(MSCollection *response, comGroupDeltaRequest *nextRequest, NSError *error))completionHandler
{
    ...
    MSURLSessionDataTask *task = [self collectionTaskWithRequest:self.mutableRequest
                                          odObjectWithDictionary:^(id responseObject){
                                                                     return [[comGroup alloc] initWithDictionary:responseObject];
                                                                 }
                                                      completion:^(MSCollection *collectionResponse, NSError *error){
                                      if(!error && collectionResponse.nextLink && completionHandler){
                                              comGroupDeltaRequest *nextRequest = [[comGroupDeltaRequest alloc] initWithURL:collectionResponse.nextLink
                                                                                                                  options:nil
                                                                                                                  client:self.client];
