...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(comSite *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comSite alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(comSite *)site
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[site dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:site]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comSite alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
