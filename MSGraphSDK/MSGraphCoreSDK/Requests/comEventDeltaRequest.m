...
@implementation comEventDeltaRequest
...
- (MSURLSessionDataTask *)executeWithCompletion:(void (^)(MSCollection *response, comEventDeltaRequest *nextRequest, NSError *error))completionHandler
{
    ...
    MSURLSessionDataTask *task = [self collectionTaskWithRequest:self.mutableRequest
                                          odObjectWithDictionary:^(id responseObject){
                                                                     return [[comEvent alloc] initWithDictionary:responseObject];
                                                                 }
                                                      completion:^(MSCollection *collectionResponse, NSError *error){
                                      if(!error && collectionResponse.nextLink && completionHandler){
                                              comEventDeltaRequest *nextRequest = [[comEventDeltaRequest alloc] initWithURL:collectionResponse.nextLink
                                                                                                                  options:nil
                                                                                                                  client:self.client];
