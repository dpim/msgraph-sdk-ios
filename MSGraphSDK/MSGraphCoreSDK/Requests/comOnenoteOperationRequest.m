...
@implementation comOnenoteOperationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(comOnenoteOperation *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comOnenoteOperation alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(comOnenoteOperation *)onenoteOperation
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[onenoteOperation dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:onenoteOperation]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comOnenoteOperation alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
