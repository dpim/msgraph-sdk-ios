...
@implementation comWorkbookRangeRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(comWorkbookRange *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comWorkbookRange alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(comWorkbookRange *)workbookRange
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbookRange dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbookRange]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comWorkbookRange alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
