...
@interface comUserReminderViewRequest()
...
@implementation comUserReminderViewRequest
...
- (MSURLSessionDataTask *)executeWithCompletion:(void (^)(MSCollection *response, comUserReminderViewRequest *nextRequest, NSError *error))completionHandler
{

    MSURLSessionDataTask *task = [self collectionTaskWithRequest:self.mutableRequest
                                          odObjectWithDictionary:^(id responseObject){
                                                                     return [[comReminder alloc] initWithDictionary:responseObject];
                                                                 }
                                                      completion:^(MSCollection *collectionResponse, NSError *error){
                                      if(!error && collectionResponse.nextLink && completionHandler){
                                              comUserReminderViewRequest *nextRequest = [[comUserReminderViewRequest alloc] initWithURL:collectionResponse.nextLink
                                                                                                                  options:nil
                                                                                                                  client:self.client];
