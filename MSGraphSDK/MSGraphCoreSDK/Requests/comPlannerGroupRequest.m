...
@implementation comPlannerGroupRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(comPlannerGroup *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comPlannerGroup alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(comPlannerGroup *)plannerGroup
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[plannerGroup dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:plannerGroup]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comPlannerGroup alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
