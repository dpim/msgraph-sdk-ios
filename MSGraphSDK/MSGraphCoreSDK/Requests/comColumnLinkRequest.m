...
@implementation comColumnLinkRequest
...
- (NSMutableURLRequest *)update:(comColumnLink *)columnLink
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[columnLink dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:columnLink]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[comColumnLink alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
